...
PARALLAX_ENCRYPT static jobjectArray mergeDexElements(JNIEnv *env,
...
PARALLAX_ENCRYPT static jobjectArray makeInMemoryDexElements(JNIEnv *env, jobject targetClassLoader) {
...
PARALLAX_ENCRYPT void combineDexElement(JNIEnv* env, jclass __unused, jobject targetClassLoader, const char* pathChs) {
...
PARALLAX_ENCRYPT void combineDexElements(JNIEnv *env, jclass klass, jobject targetClassLoader) {
    clock_t cl = clock();
    const int apiLevel = android_get_device_api_level();
...
PARALLAX_ENCRYPT void removeDexElements(JNIEnv* env,jclass __unused,jobject classLoader,jstring elementName){
...
PARALLAX_ENCRYPT jstring readAppComponentFactory(JNIEnv *env, jclass __unused) {
...
PARALLAX_ENCRYPT jobject getApplicationInstance(JNIEnv *env, jstring applicationClassName) {
...
PARALLAX_ENCRYPT jobject replaceApplication(JNIEnv *env, jclass klass, jstring realApplicationClassName){
...
PARALLAX_ENCRYPT void replaceApplicationOnActivityThread(JNIEnv *env,jclass __unused, jobject realApplication){
...
PARALLAX_ENCRYPT jobject replaceApplicationOnLoadedApk(JNIEnv *env, jclass __unused, jstring realApplicationClassName) {
    android_app_ActivityThread activityThread(env);
...
PARALLAX_ENCRYPT void read_shell_config(JNIEnv *env) {
    ...
    if(entry.has_value()) {
        ...
        if(entry_size > 0) {
            ...
            constexpr size_t CONFIG_TAG_SIZE = 32;
            const uint8_t config_magic[CONFIG_HEADER_SIZE] = {'P', 'A', 'R', '1'};
            if (entry_size <= CONFIG_HEADER_SIZE + CONFIG_TAG_SIZE ||
            ...
            auto encryption_key = hmac_sha256(aes_key.data(), aes_key.size(),
                reinterpret_cast<const uint8_t *>("Parallax/config/encryption/v1"), 29);
            auto authentication_key = hmac_sha256(aes_key.data(), aes_key.size(),
                reinterpret_cast<const uint8_t *>("Parallax/config/authentication/v1"), 33);
            const size_t authenticated_size = entry_size - CONFIG_TAG_SIZE;
            auto expected_tag = hmac_sha256(authentication_key.data(), authentication_key.size(),
                                            entry_data, authenticated_size);
            if (expected_tag.size() != CONFIG_TAG_SIZE ||
