...
bool checkWebViewInFilename(const std::string &path) {
    ...
    return lowercaseFilename.find("webview") != std::string::npos &&
           lowercaseFilename.find("vdex") != std::string::npos;;
}

size_t parallax_readlink(int fd, char *result_path, size_t path_max_len) {
...
int parallax_mprotect(void *start, void *end, int prot) {
    uintptr_t start_addr = PARALLAX_PAGE_START((uintptr_t) start);
    uintptr_t end_addr = PARALLAX_PAGE_START((uintptr_t) end - 1) + get_cache_page_size();
    size_t size = end_addr - start_addr;

    if (0 != mprotect((void *) start_addr, size, prot)) {
        DLOGW("mprotect 0x%" PRIxPTR "-0x%" PRIxPTR " fail, pagesize: %d, err: %s", start_addr,
              end_addr,
              get_cache_page_size(),
              strerror(errno));
        return -1;
...
static uint32_t readZipLength(const uint8_t *data, size_t size) {
    ...
    length = (length >> 24) | ((length & 0x00FF0000) >> 8) | ((length & 0x0000FF00) << 8) |
             (length << 24);
...
PARALLAX_ENCRYPT static std::optional<std::tuple<uint8_t *, size_t>>
...
PARALLAX_ENCRYPT void loadDexesToMemory(void *package_addr, size_t package_size) {
...
PARALLAX_ENCRYPT static size_t align_to_page_size(size_t size) {
...
void get_elf_section(Elf_Shdr *target, const char *elf_path, const char *sh_name) {
    ...
    for (int i = 0; i < ehdr->e_shnum; i++) {
        const char *section_name = reinterpret_cast<const char *>((char *) shstr_addr +
                                                                  shdr->sh_name);
...
PARALLAX_ENCRYPT void
find_symbol_in_elf_file(const char *elf_file, char *symbol_name_out, size_t max_out_len,
                        int keyword_count, ...) {
    ...
    if (elf_fp) {
        ...
        tail:
        {
            PARALLAX_FREE(lib_buffer);
            fclose(elf_fp);
        };
    } // if
}

PARALLAX_ENCRYPT int find_in_maps(int count, ...) {
    const int MAX_READ_LINE = 10 * 1024;
    char maps_path[128] = {0};
    snprintf(maps_path, ARRAY_LENGTH(maps_path), AY_OBFUSCATE("/proc/%d/maps"), getpid());
    FILE *fp = fopen(maps_path, "r");
    ...
    if (fp != nullptr) {
        ...
        while (fgets(line, sizeof(line), fp) != nullptr) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (parallax_strstr(item_name, arg) != nullptr) {
...
PARALLAX_ENCRYPT int find_in_threads_list(int count, ...) {
