...
public final class ParallaxTGUser extends AppComponentFactory {
    ...
    private AppComponentFactory resolveOriginalFactory(ClassLoader classLoader) {
        ...
        for (;;) {
            switch (state) {
                case 0x11:
                    name = ParallaxKoChummiDedo.getRealComponentFactoryName();
                    state = hop(0x22, 0x71);
                    break;
                case 0x22:
                    if (name == null || name.isEmpty()
                            || "android.app.AppComponentFactory".equals(name)
                            || ParallaxTGUser.class.getName().equals(name)) return null;
                    state = hop(0x33, 0x72);
                    break;
                case 0x33: {
    ...
    public Activity instantiateActivity(ClassLoader cl, String className, Intent intent)
        ...
        int state = ParallaxKoChummiDedo.isProtectionBlocked() ? hop(0x91, 0xB1) : hop(0x92, 0xB2);
        for (;;) {
            switch (state) {
                case 0x91: {
                    Activity activity = new ParallaxKiGF();
    ...
    public ContentProvider instantiateProvider(ClassLoader cl, String className)
            throws InstantiationException, IllegalAccessException, ClassNotFoundException {
        if (ParallaxKoChummiDedo.isProtectionBlocked()) return new ParallaxInstaUser();
        AppComponentFactory delegate = resolveOriginalFactory(cl);
    ...
    public BroadcastReceiver instantiateReceiver(ClassLoader cl, String className, Intent intent)
            throws InstantiationException, IllegalAccessException, ClassNotFoundException {
        if (ParallaxKoChummiDedo.isProtectionBlocked()) return new ParallaxSnapUser();
        AppComponentFactory delegate = resolveOriginalFactory(cl);
    ...
    public Service instantiateService(ClassLoader cl, String className, Intent intent)
            throws InstantiationException, IllegalAccessException, ClassNotFoundException {
        if (ParallaxKoChummiDedo.isProtectionBlocked()) return new ParallaxYTUser();
        AppComponentFactory delegate = resolveOriginalFactory(cl);
