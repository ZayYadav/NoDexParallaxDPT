...
# Fixed custom-VM bridge. The target APK trampolines are outside R8's graph.
...
# Source-level framework ABI referenced by the protected manifest.
-keepclassmembers class com.parallax.shell.Parallax2 {
    public <init>();
    public *** instantiate*(...);
}

# Internal compatibility alias only; it may be removed or renamed by R8.
